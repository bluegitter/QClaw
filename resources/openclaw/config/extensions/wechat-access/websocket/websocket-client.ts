...
export class WechatAccessWebSocketClient {
   ...
   * 避免特殊字符导致的 URL 解析问题。
   *
   * 最终格式：ws://host:port/?token={token}&guid={guid}&user_id={userId}
   */
  ...
  private handleError = (error: Error): void => {
    ...
    // 403 握手拒绝通常意味着 token / guid / user_id 不被服务端接受，
    // 继续本地重连只会重复刷日志，等待配置修正后再重启连接即可。
    if (error.message.includes("Unexpected server response: 403")) {
      console.error(
        `${this.logPrefix} 握手被服务端拒绝(403)，停止自动重连，请检查 token、guid、user_id 是否为服务端分配的有效值`
      );
      this.state = "disconnected";
      this.clearReconnectTimer();
    }
